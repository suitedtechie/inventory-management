"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createPurchaseOrderItem = createPurchaseOrderItem;
exports.getPurchaseOrderItemById = getPurchaseOrderItemById;
exports.getAllPurchaseOrderItems = getAllPurchaseOrderItems;
exports.updatePurchaseOrderItem = updatePurchaseOrderItem;
exports.deletePurchaseOrderItem = deletePurchaseOrderItem;
// src/services/purchaseOrderItemService.ts
const index_1 = require("../db/index");
function createPurchaseOrderItem(data) {
    return __awaiter(this, void 0, void 0, function* () {
        return index_1.prisma.purchaseOrderItem.create({ data });
    });
}
function getPurchaseOrderItemById(id) {
    return __awaiter(this, void 0, void 0, function* () {
        return index_1.prisma.purchaseOrderItem.findUnique({
            where: { id },
            include: {
                purchaseOrder: true,
                item: true,
            },
        });
    });
}
function getAllPurchaseOrderItems() {
    return __awaiter(this, void 0, void 0, function* () {
        return index_1.prisma.purchaseOrderItem.findMany({
            include: {
                purchaseOrder: true,
                item: true,
            },
        });
    });
}
function updatePurchaseOrderItem(id, data) {
    return __awaiter(this, void 0, void 0, function* () {
        return index_1.prisma.purchaseOrderItem.update({
            where: { id },
            data,
        });
    });
}
function deletePurchaseOrderItem(id) {
    return __awaiter(this, void 0, void 0, function* () {
        return index_1.prisma.purchaseOrderItem.delete({
            where: { id },
        });
    });
}
